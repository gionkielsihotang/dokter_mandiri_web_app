...
CREATE TABLE Rekam_Medis (
    ...
    visit_id INT UNIQUE NOT NULL, -- UNIQUE untuk memastikan relasi 1-to-1 dengan Kunjungan
    FOREIGN KEY (visit_id) REFERENCES Kunjungan(visit_id) ON DELETE CASCADE
);
...
CREATE TABLE Batch (
    ...
    FOREIGN KEY (gr_id) REFERENCES Penerimaan_Barang(gr_id) ON DELETE CASCADE,
...
CREATE TABLE Hasil_Penunjang (
    ...
    FOREIGN KEY (order_id) REFERENCES Order_Penunjang(order_id) ON DELETE CASCADE
);
...
CREATE TABLE Detail_Resep (
    ...
    obat_id INT NOT NULL,
    FOREIGN KEY (resep_id) REFERENCES Resep(resep_id) ON DELETE CASCADE,
    FOREIGN KEY (obat_id) REFERENCES Obat(obat_id) ON DELETE CASCADE
);

CREATE TABLE Transaksi_Stok (
    transaksi_id INT PRIMARY KEY AUTO_INCREMENT, -- Menambahkan PK yang tidak tertulis di ERD
    tanggal DATETIME,
